...
async function main() {
    ...
    app.get('/students/:aridno', async (req, res) => {
        ...
        try {
            ...
            const classesData = await Promise.all(classrooms.map(async (classroom) => {
            ...
            student.classesData = classesData.filter(classInfo => classInfo !== null);
    ...
    app.post("/classes", async (req, res) => {
        const { name, semester, section, shift, day, classroom, classtime, teacher, course } = req.body;
        if (!name || !semester || !section || !shift || !day || !classroom || !classtime || !teacher || !course) {
            return res.status(400).json({ message: "All fields are required" });
        }
        ...
        try {
            ...
            if (existingClass) {
                return res.status(400).json({ message: "A class with the same classroom, semester, section, shift, and day already exists at this time." });
            }
            ...
            if (error.code === 'P2002') { // Prisma unique constraint violation error code
                res.status(400).json({ message: "Unique constraint failed. A class with the same classroom already exists at this time." });
            } else {
    ...
    app.put("/classes/:id", async (req, res) => {
        ...
        const { name, semester, section, shift, day, classroom, classtime, teacher, course } = req.body;
        if (isNaN(classId) || !name || !semester || !section || !shift || !day || !classroom || !classtime || !teacher || !course) {
            return res.status(400).json({ message: "All fields are required" });
        }
